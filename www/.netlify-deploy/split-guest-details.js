...
// netlify/functions/split-guest-details.js
...
exports.handler = async function(event) {
  ...
  try {
    ...
    var result = await supabase.from("split_participants").select("id, email, display_name, amount_cents, status, split_payment_id, split_payments(package_id, total_amount_cents, status, expires_at)").eq("id", participantId).single();
    ...
    if (participant.split_payments && participant.split_payments.package_id) {
      var pkgResult = await supabase.from("maintenance_packages").select("title").eq("id", participant.split_payments.package_id).single();
      if (pkgResult.data) {
