...
exports.handler = async function(event) {
  ...
  try {
    ...
    var result = await supabase.from("provider_referral_codes").select("id, code, code_type, provider_id, skip_identity_verification, platform_fee_exempt, is_active").eq("code", upperCode).eq("is_active", true).single();
    ...
    if (referralCode.provider_id) {
      var profileResult = await supabase.from("profiles").select("full_name, business_name").eq("id", referralCode.provider_id).single();
