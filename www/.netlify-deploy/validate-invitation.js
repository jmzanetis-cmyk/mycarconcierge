...
// netlify/functions/validate-invitation.js
...
exports.handler = async function(event) {
  ...
  try {
    ...
    var result = await supabase.from("provider_invitations").select("id, email, role, provider_id, expires_at").eq("token", token).is("accepted_at", null).gt("expires_at", (/* @__PURE__ */ new Date()).toISOString()).single();
    ...
    if (invitation.provider_id) {
      var profileResult = await supabase.from("profiles").select("full_name, business_name").eq("id", invitation.provider_id).single();
