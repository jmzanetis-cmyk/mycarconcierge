...
// netlify/functions/email-tracking.js
...
exports.handler = async function(event) {
  ...
  if (supabase) {
    try {
      var result = await supabase.from("outreach_messages").select("id, lead_id, opened_at, clicked_at").eq("id", msgId).single();
      var msg = result.data;
      if (action === "open" && msg && !msg.opened_at) {
        await supabase.from("outreach_messages").update({ opened_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", msgId);
      ...
      if (action === "click" && msg && !msg.clicked_at) {
        await supabase.from("outreach_messages").update({ clicked_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", msgId);
        await supabase.from("outreach_activity_log").insert({
          lead_id: msg.lead_id,
          message_id: msgId,
          event_type: "clicked",
