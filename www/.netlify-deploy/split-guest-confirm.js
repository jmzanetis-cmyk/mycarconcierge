...
// netlify/functions/split-guest-confirm.js
...
exports.handler = async function(event) {
  ...
  try {
    ...
    var result = await supabase.from("split_participants").select("id, status, payment_intent_id, split_payment_id").eq("id", participantId).single();
    ...
    var allParticipants = await supabase.from("split_participants").select("id, status").eq("split_payment_id", participant.split_payment_id);
    if (allParticipants.data) {
      ...
      if (allPaid) {
        var splitResult = await supabase.from("split_payments").update({ status: "complete" }).eq("id", participant.split_payment_id).select("package_id").single();
        if (splitResult.data && splitResult.data.package_id) {
          await supabase.from("maintenance_packages").update({
            status: "payment_held",
