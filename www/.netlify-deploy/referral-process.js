...
// netlify/functions/referral-process.js
...
exports.handler = async function(event) {
  ...
  try {
    ...
    var codeResult = await supabase.from("provider_referral_codes").select("id, code, code_type, provider_id, skip_identity_verification, platform_fee_exempt, is_active, uses_count").eq("code", upperCode).eq("is_active", true).single();
    ...
    var insertResult = await supabase.from("provider_referrals").insert({
    ...
    try {
      ...
      await supabase.from("provider_referral_codes").update({ uses_count: (referralCodeData.uses_count || 0) + 1 }).eq("id", referralCodeData.id);
    }
    var providerName = "";
    if (referralCodeData.provider_id) {
      var profileResult = await supabase.from("profiles").select("full_name, business_name").eq("id", referralCodeData.provider_id).single();
