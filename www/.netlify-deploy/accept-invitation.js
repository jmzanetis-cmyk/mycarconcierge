...
// netlify/functions/accept-invitation.js
...
exports.handler = async function(event) {
  ...
  try {
    ...
    var invResult = await supabase.from("provider_invitations").select("id, email, role, provider_id, expires_at").eq("token", token).is("accepted_at", null).gt("expires_at", (/* @__PURE__ */ new Date()).toISOString()).single();
    ...
    await supabase.from("profiles").update({ team_provider_id: invitation.provider_id }).eq("id", user.id);
