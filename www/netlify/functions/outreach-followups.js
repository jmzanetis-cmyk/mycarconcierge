...
exports.handler = async function(event, context) {
  console.log('[OutreachEngine] Scheduled follow-ups trigger fired');
  ...
  try {
    const response = await fetch(`${siteUrl}/.netlify/functions/outreach-followups-background`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ triggered_at: new Date().toISOString() })
    });
    console.log('[OutreachEngine] Follow-ups background function invoked, status:', response.status);
  } catch (err) {
    console.error('[OutreachEngine] Failed to invoke follow-ups background function:', err.message);
  }
