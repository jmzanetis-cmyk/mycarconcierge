...
const STRESS_TAG = process.env.STRESS_TAG || ('stress-bgc-broadcast-' + Date.now());
...
async function cleanup() {
  ...
  // Task #396 — emit manifest BEFORE deletes so verify-stress-cleanup.js
  // can confirm zero leftover rows in profiles / email_unsubscribes /
  // outreach_leads / agent_memory tagged with this run.
  writeStressManifest({
    test: 'bgc-broadcast',
    stress_tag: STRESS_TAG,
    test_start_iso: testStartIso,
    stress_domain: STRESS_DOMAIN,
    profile_ids: ids,
    emails,
  });
  if (emails.length > 0) {
