...
const STRESS_TAG = process.env.STRESS_TAG || ('stress-care-plan-' + Date.now());
...
async function cleanup(planIds) {
  // Task #396 — write a manifest of seeded scopes BEFORE any deletes so
  // scripts/verify-stress-cleanup.js can prove zero leftover rows survived.
  writeStressManifest({
    test: 'care-plan-lifecycle',
    stress_tag: STRESS_TAG,
    test_start_iso: testStartIso,
    care_plan_ids: planIds,
    provider_ids: Array.from(seededProviderIds),
  });
