...
- **Fleet SaaS (#88)**: Subscription status card + vehicle limit usage bar in fleet dashboard. Upgrade banner when near tier limit. `GET /api/fleet/subscription` and `POST /api/fleet/check-limits` endpoints with plan-based vehicle/driver limits (Starter: 5 vehicles/5 drivers, Pro: 25 vehicles/25 drivers, Business: unlimited). Fleet invite system: `POST /api/fleet/invite`, `GET /api/fleet/invite/:token`, `POST /api/fleet/invite/:token/accept` with email delivery via Resend, driver limit enforcement at invite send AND accept time (prevents over-provisioning via pending invites), email match guard, and 7-day token TTL. `POST /api/fleet/import-vehicles` for server-side CSV import with plan limit enforcement returning 402 on limit exceeded. Dedicated pages: `fleet-landing.html` (marketing at `/fleet` with fleet-specific CTAs to `/fleet/signup`), `fleet-signup.html` (4-step onboarding: account → company → plan → atomic fleet creation, sets `account_type='fleet'`), `fleet-join.html` (invite acceptance), `fleet-driver.html` (mobile driver portal at `/fleet/driver`). URL routes: `/fleet`, `/fleet/signup`, `/fleet/driver`, `/fleet/join` all wired in server URL redirects. CSV vehicle import via drag-and-drop modal with server-side enforcement. Section nav wiring: `loadFleetSubscription()` fires when fleet tab opens.
