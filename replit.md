...
- **AI Ops Agent Fleet**: Multi-agent system (Orchestrator, Analyst, Matchmaker, Treasurer, Gatekeeper, Concierge, Advocate, Hunter) for autonomous administration, social acquisition, and dispute resolution. **Matchmaker apply parity (Task #153)**: when an admin clicks "Accept winning bid" on `/admin/agent-fleet-detail.html`, `applyMatchmakerRank` (`netlify/functions/agent-fleet-admin.js`) now fans out service-role notifications + Resend emails on top of the existing plan_bids/care_plans updates — winning provider gets a `bid_accepted` notification + "Your bid was accepted" email, every losing provider gets a `bid_not_selected` notification + courteous "thanks, not selected" email (best-effort, skipped silently if `RESEND_API_KEY` is unset), and the member gets an `auction_awarded` notification + "Your auction has been awarded" email naming the winning provider, plan title, and amount with a deep-link to authorize escrow. Delivery summary (`{winner_notified, member_notified, loser_notified_count, winner_emailed, member_emailed, loser_emailed_count, errors}`) is persisted into the audit `agent_actions.decision.notifications` row so admins can see what actually went out. Notification fan-out is best-effort: failures never roll back the bid acceptance which is committed first. Verified by `node scripts/matchmaker-award-notify-test.js` (29 checks: bid status transitions, notification rows, email recipients, audit summary, RESEND_API_KEY-disabled fallback). **Gatekeeper enablement (Task #126)**: handler + producers + Apply/Suspend queue + prompt-edit admin all shipped in prior tasks (#123/#127/#128); the supervised production rollout is gated by the SQL `supabase/migrations/20260424_enable_gatekeeper.sql` (single `UPDATE public.agents SET enabled=true, autonomy='propose' WHERE slug='gatekeeper'` — apply via Supabase SQL Editor, or click the toggle at `/admin/agent-fleet.html`). Daily spend cap stays at the seeded $3. Verification: `node scripts/gatekeeper-enable-smoke.js` (env: SITE_URL, ADMIN_PASSWORD, SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY) fires one synthetic event of each subscribed type (`provider.applied`, `provider.bgc_completed`, `provider.flagged`), forces an orchestrator tick, polls `agent_actions` for proposed rows, and prints a recommendation/confidence/cost summary. Full operator playbook + 24h observation + rollback in `docs/gatekeeper-enablement-runbook.md`. Rollback is a single SQL line (`enabled=false`).
