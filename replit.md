...
- **MCC Verified Copy & Badge Rollout (Task #114)**: Customer-facing rebrand of the BGC compliance system. New shared modules `www/bgc-copy.js` (single source of truth for verbatim PDF copy: branding, customer, provider, onboarding, badge, homepage, legal — now ships parallel translations for all 7 supported locales: en, es, fr, el, zh, hi, ar; placeholders `$[XX]` and `78%` preserved literal; `MCC_BGC_COPY_GET_LANGUAGE_TAG()` exposes the active BCP-47 tag) and `www/bgc-badge.js` (vanilla JS component with `compact` / `full` / `card` / `detail` variants plus a `What does MCC Verified mean?` modal, self-injecting styles; `fmtMonth` now formats month/year using the active locale's BCP-47 tag with safe en-US fallback). Surfaces wired in this pass: provider directory cards (compact badge swap + new "Show only Verified Providers" toggle), provider dashboard overview (4-state status card via `bgc-verified-status.js` — Active / At Risk / Inactive / Not Enrolled), Car Club provider profile (verified card or neutral fallback), homepage trust section, and `/marketing/providers.html` 3-column "Verified providers book more jobs" block. Two Resend email templates added under `netlify/email-templates/` (customer announcement, provider launch). Read-only consumer of existing `profiles.bgc_*` columns — no schema changes. The public provider detail page (`/p/:slug`) badge is deferred to a follow-up. Onboarding-provider 4-step BGC flow shipped in Task #115 — `www/onboarding-provider.html` now inserts four panels (data-step 13–16) using verbatim copy from `MCC_BGC_COPY.onboarding`, batched per-employee POSTs to `/api/bgcheck/initiate` with `Promise.allSettled`, skip-able from any panel, stays on consent step with inline error if all initiations fail, and only advances to the success screen (data-step 17, renumbered) when at least one check succeeds. `totalSteps` bumped 13→17. `<!-- TODO ES -->` markers placed on all new strings; ES translations pending. Pricing placeholder `$[XX]` and stat `78%` remain literal until real numbers are confirmed.
