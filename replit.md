...
- **Merch Manager (Admin Tool)**: Admin-only interface for creating and managing Printful products directly from the admin dashboard. Features catalog browser (t-shirts, hoodies, hats, drinkware, stickers, phone cases, bags), product creator with color/size variant selection, design URL input, retail price setting, and store product management with delete capability. API endpoints: `/api/admin/printful/catalog`, `/api/admin/printful/products`. All endpoints require admin authentication.
