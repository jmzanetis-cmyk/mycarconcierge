...
- **Founder Referral Commission System**: Both members and providers automatically become "founders" who can refer new providers. Members receive MF-prefixed codes (stored in `member_founder_profiles`), providers receive PR-prefixed codes (stored in `provider_referral_codes`). Welcome emails include QR codes linking to provider signup. When referred providers purchase bid packs, the referrer earns 50% lifetime commission via `record_bid_pack_commission` RPC, tracked in `founder_referrals` and `founder_commissions` tables.
