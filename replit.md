...
- **Fleet SaaS (#88)**: Subscription status card + vehicle limit usage bar in fleet dashboard. Upgrade banner when near tier limit. `GET /api/fleet/subscription` and `POST /api/fleet/check-limits` endpoints with plan-based vehicle/driver limits (Starter: 10/5, Pro: 50/25, Business: unlimited). Fleet invite system: `POST /api/fleet/invite`, `GET /api/fleet/invite/:token`, `POST /api/fleet/invite/:token/accept` with email delivery via Resend, limit enforcement, and 7-day token TTL. Dedicated pages: `fleet-join.html` (invite acceptance), `fleet-driver.html` (mobile driver portal with vehicle info, service history, and issue reporting), `fleet-landing.html` (marketing landing page with pricing). CSV vehicle import with drag-and-drop, column validation, progress bar, and bulk Supabase insert. Section nav wiring: `loadFleetSubscription()` fires when fleet tab opens.
