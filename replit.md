...
- **AI Ops Agent Fleet**: Multi-agent system (Orchestrator, Analyst, Matchmaker, Treasurer, Gatekeeper, Concierge, Advocate, Hunter, Director, Promoter) for autonomous administration, social acquisition, and dispute resolution. Each agent's admin detail page (`/admin/agent-fleet-detail.html?slug=…`) includes editable prompt versioning with rollback (Task #128) and a side-by-side diff viewer (Task #176): clicking any row in "Recent versions" diffs that version against the active version (or against the immediately-preceding version when the active row itself is clicked), and a "Compare" picker can diff any two arbitrary versions. Bodies are loaded on-demand via `GET /agents/:slug/prompt/:version` (deliberately omitted from the list payload to keep long Sonnet prompts cheap to enumerate). The diff renders client-side via the `diff` package (CDN, no build step) with simple +/− line markers. Includes a supervised rollout for the Gatekeeper agent. The **Director** is an acquisition-focused chief-of-staff that runs every 15 minutes (`netlify/functions/agent-director-scheduled.js`, cron in `netlify.toml`), watches whether the other agents are pulling customers through the funnel (Gatekeeper failures, Promoter draft backlog/idleness, Hunter unscored leads, social-monitor dry runs, Matchmaker silent on closed auctions, signup funnel drops vs trailing 7d avg), and pages the admin via SMS (Twilio → `ADMIN_ALERT_PHONE`) + email (Resend → `ADMIN_EMAIL`) with the *specific next action* (e.g. "Top up Anthropic credits at https://console.anthropic.com/settings/billing"). Uses no LLM tokens — pure DB scans. Alert state lives in `agent_director_alerts` with a partial unique index on `alert_key WHERE resolved_at IS NULL` for natural dedupe; same-key re-fires only re-page on `severity='critical'` and only after `dedupe_repage_hours` (default 6h). Quiet hours (default 02:00–11:00 UTC ≈ 10pm–7am ET) suppress non-critical SMS/email; critical alerts always page through. Daily digest fires at the 11:00 UTC tick (~7am ET) regardless of findings so silence is never ambiguous. Thresholds + quiet-hours overridable per-deploy via `agents.config` jsonb. Migration: `supabase/migrations/20260429b_agent_director.sql`.
...
- **Stress Test Suite**: 21 `www/stress-test-*.js` scripts cover concurrent traffic against high-impact surfaces. Task #227 added 5 new tests for surfaces shipped since the last expansion: `stress-test-care-plan-lifecycle.js` (race-tests `/api/care-plans/:id/complete` vs `/dispute` for double-completion-row integrity), `stress-test-bgc-webhook.js` (in-process replay of `netlify/functions/background-check-webhook` with valid HMAC signatures — skips cleanly with exit 0 when `BGC_WEBHOOK_SECRET` is unset), `stress-test-bgc-broadcast.js` (spawns `scripts/send-bgc-launch-broadcast.js --dry-run` in 4 progressively-larger phases and verifies throttle + zero log-table writes in dry-run mode), `stress-test-survey-intake.js` (concurrent public POST `/api/survey/response` with all 22 dimensions per payload, integrity-samples persisted rows), and `stress-test-shop-checkout.js` (2/3 reads + 1/3 checkouts against `/api/shop/products` + `/api/shop/checkout`, treats 401/403 from the 2FA gate as expected and verifies no duplicate `stripe_session_id` rows). Each follows the standard architecture: 50000-element Float32Array reservoir, `--concurrency`/`--duration`/`--ramp-up` CLI params, 4-phase loader (Ramp 30% → Sustained → Spike 2x → Cool-down), per-test PASS/FAIL block with exit 0/1, sim accounts (`@mcc-sim.test` / `SimPass123!`), and `try/finally` cleanup keyed off a per-run `STRESS_TAG`.
