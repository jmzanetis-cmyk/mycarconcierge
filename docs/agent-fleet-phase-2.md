...
### 3.5 Prompt template versioning (P1) — ✅ SHIPPED (Task #128)
Originally scoped as moving prompts into `agents.config.prompt_template` (jsonb). Shipped as a dedicated `public.agent_prompt_versions` table instead (`supabase/migrations/20260424_agent_prompt_versions.sql`) — `(agent_slug, version, body, notes, is_active, created_at, created_by)` with a partial unique index that enforces at most one active row per agent. The shared runtime exposes `loadActivePrompt(supabase, slug, fallback)` which is consumed by every Claude-calling specialist (Gatekeeper, Matchmaker, Hunter, Promoter); when no row is marked active the file-based default constant is used unchanged. Per-process cache (30s TTL) is invalidated on save/rollback via `clearPromptCache(slug)` so warm Netlify containers pick up edits within a tick. Admin UI lives on each `/admin/agent-fleet-detail.html` page (Prompt card + version history with one-click rollback). Prompt edits and rollbacks each write a `prompt.update` / `prompt.rollback` row into `agent_actions` (`status='executed'`, `autonomy_used='admin'`) so changes show up in the agent's own activity feed. **Out of scope (intentionally deferred):** A/B testing prompts, shadow runs, prompt templating with variables — these stay Phase 3.
