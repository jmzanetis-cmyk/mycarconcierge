...
| `} catch (creditErr)` in Stripe webhook bid-credits (line ~10821) | Returning 200 to Stripe is required so Stripe doesn't retry the whole webhook (which would re-process the payment). The credit-add failure is logged. **Caveat:** out of scope for this audit, but the lack of a retry queue here is tracked separately — the audit only covers DB-error swallows in API routes, not webhook retry semantics. |
