...
Two founder-commission models are agreed as of 2026-09-19 (standard + Chris). A third (Rossy) was previously listed here as "CONFIRMED FINAL 2026-07-10" but Rossy has not in fact agreed to those terms — see the amendment note below. All agreed tiers must be captured whenever commission logic is built or enforced (payouts, dashboards, statement generation, referral-attribution reads).
...
| ~~**Rossy Mateo**~~ *(PROPOSED — not agreed as of 2026-09-19. Do not implement.)* | ~~75% of bid-credit pack revenue~~ | ~~Providers referred by Rossy~~ | ~~Perpetual (lifetime of the referred account)~~ | ~~Third distinct model~~ — see amendment note below |

**Rossy Mateo — amendment 2026-09-19:** the "CONFIRMED FINAL 2026-07-10" note in earlier revisions of this file was wrong. Rossy has NOT agreed to these terms. This row is proposed, not agreed, and must not be implemented. Do not seed a `member_founder_profiles` row for Rossy, do not seed a `commission_overrides` row, and do not treat 75% / perpetual as a live tier anywhere in code or data. If terms are agreed with Rossy in the future, the correct action is a **row insert into `commission_overrides`** (data, not code) — no code path needs to know about her.

**Implementation flag:** commission overrides are **data, not code**. The subscriptions build spec introduces a `commission_overrides` table (`referrer_id, rate, window_months`) seeded with Chris only (90%, perpetual → `window_months = null`). The default path is `50% for 12 months`. Any code path that reads commission rates must consult that table and fall back to the default; **do not hard-code any founder's rate or duration**, and do not add special-cases per person. Adding a new agreed tier in the future is a single row insert.
...
| **Commission calculation** — per-founder, honoring per-account duration windows | ✅ **BUILT (partial).** `record_bid_pack_commission` RPC (`20260515b:76`, v2 in `20260603b`) reads `referred_by_founder_id`, joins `member_founder_profiles` for founder id + rate, records commission row. `commission_rate_history` audit table exists (`20260428h:47-58`). **Duration windows are NOT enforced today** — standard founders are earning on gross in perpetuity, contrary to the 50% / 12-month spec; see the subscriptions build spec for the corrective `accrueCommission` path that generalises this across packs and plans and enforces the 12-month window via `commission_overrides` | Commissions are being written on bid-pack sales today; dashboard just needs to read them back |
...
1. ~~Confirm Rossy Mateo's `member_founder_profiles` row exists.~~ **Removed 2026-09-19** — Rossy has not agreed to any terms; do not seed her row. See the amendment note in §2a.
2. **Audit + backfill historic Chris referrals attributed pre-2026-05-23.** Migration `20260523b` fixed the `referred_by_founder_id` FK, but its comment states the column was NULL in prod at that time. The signup write-path only fires when a new signup uses `?ref=CODE` — anyone signed up before 2026-05-23 (or without a ref link) has `referred_by_founder_id = NULL` regardless of who actually referred them. For Chris's historic referrals, run a `SELECT id, created_at, referred_by_founder_id FROM profiles WHERE created_at < '2026-05-23' AND …` audit, then a manual `UPDATE profiles SET referred_by_founder_id = <chris_uid> WHERE id = <provider_uid>` per confirmed referral. Every un-backfilled historic referral is a commission Chris never gets credited for.
