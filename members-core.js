...
// ========== AI PREDICTIVE MAINTENANCE ===// ========== RENDER VEHICLES (Core) ==========
...
async function mountContributeCardElement() {
  try {
    const stripeInstance = typeof initStripe === 'function' ? await initStripe() : (typeof Stripe !== 'undefined' ? Stripe(globalThis.stripePublishableKey || '') : null);
    if (!stripeInstance) {
    ...
    globalThis._contributeStripe = stripeInstance;
    ...
    globalThis._contributeCardElement = card;
...
async function submitContribution(packageId) {
  ...
  if (!amount || amount < 1) { showToast('Please enter a valid amount (minimum $1)', 'error'); return; }
  if (amount > 5000) { showToast('Maximum contribution is $5,000', 'error'); return; }
  const stripeInstance = globalThis._contributeStripe;
  const cardElement = globalThis._contributeCardElement;
  if (!stripeInstance || !cardElement) { showToast('Payment system not ready. Please close and try again.', 'error'); return; }
