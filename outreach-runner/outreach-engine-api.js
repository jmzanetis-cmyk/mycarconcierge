...
async function runEngineCycle(supabase) {
  try {
    ...
    const { data: alreadyScoredIds } = await supabase
      .from('opportunity_pipeline')
      .select('lead_id');
    const scoredSet = new Set((alreadyScoredIds || []).map(r => r.lead_id));
...
- For investors: do NOT make specific ROI promises, earnings claims, or securities claims. Direct investors to the Wefunder campaign page at https://wefunder.com/my.car.concierge where they can learn more and invest. Also mention mycarconcierge.com for the product itself
- End with a simple call to action pointing to mycarconcierge.com (or wefunder.com/my.car.concierge for investor leads)
