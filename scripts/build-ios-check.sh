...
if [ ${#ADMIN_FILES[@]} -eq 0 ]; then
  ...
  for f in "${ADMIN_FILES[@]}"; do
    check_fail "Admin file still present: $f"
  done
fi
...
if [ ${#OUTREACH_FILES[@]} -eq 0 ]; then
  ...
  for f in "${OUTREACH_FILES[@]}"; do
    check_fail "Outreach file still present: $f"
  done
fi
...
SERVER_FILES=()
for f in server.js simulate.js simulate-platform.js playwright.config.js \
          seed-test-data.js electron.js; do
...
if [ ${#SERVER_FILES[@]} -eq 0 ]; then
  ...
  for f in "${SERVER_FILES[@]}"; do
    check_fail "Server file still present: $f"
  done
fi
...
if [ -z "$STRESS_FILES" ]; then
  ...
  check_fail "Stress-test files still present: $(echo "$STRESS_FILES" | xargs -I{} basename {} | tr '\n' ' ')"
fi
...
if [ -z "$SQL_FILES" ]; then
  ...
  check_fail "SQL files still in build: $(echo "$SQL_FILES" | tr '\n' ' ')"
fi
...
MARKETING_FILES=()
for f in ad-deck.html member-founder-deck.html member-founder.html \
          ...
          MCC-Brand-Assets.html member-founder-agreement.html \
...
echo ""
echo "[9] Checking admin redirect patch in login.js..."
...
REQUIRED_FILES=(members.html members.js login.html signup-member.html onboarding-member.html \
                ...
                stripeutils.js mcc-config.js supabaseclient.js sw.js manifest.json)
MISSING_REQUIRED=()
for f in "${REQUIRED_FILES[@]}"; do
  [ ! -f "$BUILD_DIR/$f" ] && MISSING_REQUIRED+=("$f")
done
if [ ${#MISSING_REQUIRED[@]} -eq 0 ]; then
  check_pass "All required consumer files present (members, payments, auth)"
else
