...
// ========== BID INSIGHTS ==========
let bidInsightsData = null;
bidInsightsLoaded = false;
let bidInsightsLoadedAt = 0;
const BID_INSIGHTS_TTL = 5 * 60 * 1000;

function escapeAIText(str) {
...
async function loadBidInsights(forceRefresh = false) {
  const isStale = bidInsightsLoaded && (Date.now() - bidInsightsLoadedAt > BID_INSIGHTS_TTL);
  if (bidInsightsLoaded && !forceRefresh && !isStale) return;
  
  const panel = document.getElementById('bid-insights-panel');
  const content = document.getElementById('bid-insights-content');
  if (!panel || !content) return;
  
  panel.style.display = 'block';
  content.innerHTML = `
    <div style="text-align:center;padding:30px;">
      <div style="margin:0 auto 12px;width:32px;height:32px;border:3px solid var(--border-subtle);border-top-color:var(--accent-blue);border-radius:50%;animation:spin 0.8s linear infinite;"></div>
      <p style="color:var(--text-muted);font-size:0.88rem;">Analyzing your bidding performance...</p>
    </div>
  `;
  ...
  try {
    ...
    const data = await response.json();
    bidInsightsData = data;
    bidInsightsLoaded = true;
    bidInsightsLoadedAt = Date.now();
    
    if (!data.insights) {
      content.innerHTML = `
        <div style="text-align:center;padding:24px;">
          <p style="color:var(--text-muted);font-size:0.9rem;">${data.message || 'Submit bids to see AI-powered insights about your bidding performance.'}</p>
        </div>
      `;
      return;
    }
    
    renderBidInsights(data);
  } catch (err) {
    console.error('Error loading bid insights:', err);
    content.innerHTML = `
      <div style="text-align:center;padding:24px;">
        <p style="color:var(--text-muted);font-size:0.9rem;">Unable to load insights right now. Try again later.</p>
      </div>
    `;
  }
}

function renderBidInsights(data) {
  const content = document.getElementById('bid-insights-content');
  if (!content) return;
  
  const { insights, stats, categoryBreakdown } = data;
  
  const winRateColor = stats.winRate >= 50 ? 'var(--accent-green)' : stats.winRate >= 25 ? 'var(--accent-gold)' : 'var(--accent-red)';
  
  let html = `
    <div class="bid-insights-stats">
      <div class="bid-insights-stat">
        <div class="bid-insights-stat-value">${stats.totalBids}</div>
        <div class="bid-insights-stat-label">Total Bids</div>
      </div>
      <div class="bid-insights-stat">
        <div class="bid-insights-stat-value" style="color:var(--accent-green);">${stats.wonBids}</div>
        <div class="bid-insights-stat-label">Won</div>
      </div>
      <div class="bid-insights-stat">
        <div class="bid-insights-stat-value" style="color:var(--accent-red);">${stats.lostBids}</div>
        <div class="bid-insights-stat-label">Lost</div>
      </div>
      <div class="bid-insights-stat">
        <div class="bid-insights-stat-value" style="color:${winRateColor};">${stats.winRate}%</div>
        <div class="bid-insights-stat-label">Win Rate</div>
      </div>
    </div>
  `;
  
  if (insights.summary) {
    html += `<div class="bid-insights-summary">${escapeAIText(insights.summary)}</div>`;
  }
  
  if (categoryBreakdown && categoryBreakdown.length > 0) {
    html += '<div class="bid-insights-chips">';
    categoryBreakdown.forEach(cat => {
      const decided = cat.won + cat.lost;
      const catWinRate = decided > 0 ? Math.round((cat.won / decided) * 100) : 0;
      const compClass = catWinRate >= 50 ? 'strong' : catWinRate >= 25 ? 'moderate' : 'weak';
      const catName = escapeAIText(typeof formatCategory === 'function' ? formatCategory(cat.category) : cat.category);
      html += `<span class="bid-insight-chip ${compClass}">${catName}: ${catWinRate}% win rate</span>`;
    });
    html += '</div>';
  }
  
  if (insights.categoryRecommendations && insights.categoryRecommendations.length > 0) {
    html += '<div class="bid-insights-recommendations">';
    insights.categoryRecommendations.forEach(rec => {
      const compIcon = rec.competitiveness === 'strong' ? '&#9650;' : rec.competitiveness === 'weak' ? '&#9660;' : '&#9654;';
      const compColor = rec.competitiveness === 'strong' ? 'var(--accent-green)' : rec.competitiveness === 'weak' ? 'var(--accent-red)' : 'var(--accent-gold)';
      const catName = escapeAIText(typeof formatCategory === 'function' ? formatCategory(rec.category) : rec.category);
      const priceMin = rec.suggestedPriceRange && typeof rec.suggestedPriceRange.min === 'number' ? Math.round(rec.suggestedPriceRange.min) : null;
      const priceMax = rec.suggestedPriceRange && typeof rec.suggestedPriceRange.max === 'number' ? Math.round(rec.suggestedPriceRange.max) : null;
      html += `
        <div class="bid-insights-rec-card">
          <h4><span style="color:${compColor};">${compIcon}</span> ${catName}</h4>
          <p>${escapeAIText(rec.recommendation)}</p>
          ${priceMin !== null && priceMax !== null ? `<div class="price-range">Suggested: $${priceMin} - $${priceMax}</div>` : ''}
        </div>
      `;
    });
    html += '</div>';
  ...
  if (insights.topTip) {
    html += `
      <div class="bid-insights-top-tip">
        <span style="font-size:1.3rem;flex-shrink:0;">&#128161;</span>
        <div>
          <div style="font-size:0.78rem;font-weight:600;color:var(--accent-blue);margin-bottom:4px;">TOP TIP</div>
          <p>${escapeAIText(insights.topTip)}</p>
        </div>
      </div>
    `;
  }
  
  if (insights.timingAdvice) {
    html += `<p style="margin-top:12px;font-size:0.83rem;color:var(--text-muted);"><strong>Timing:</strong> ${escapeAIText(insights.timingAdvice)}</p>`;
  }
  
  content.innerHTML = html;
}

async function loadAIPriceSuggestion(packageId, category) {
  const container = document.getElementById('calc-ai-price-suggestion');
  if (!container) return;
  ...
  container.innerHTML = `
    <div class="calc-ai-suggestion-title">
  ...
  try {
    ...
    if (!response.ok) throw new Error('Failed to load price advice');
    
    const data = await response.json();
    
    if (data.aiSuggestedRange) {
      const rangeMin = typeof data.aiSuggestedRange.min === 'number' ? Math.round(data.aiSuggestedRange.min) : 0;
      const rangeMax = typeof data.aiSuggestedRange.max === 'number' ? Math.round(data.aiSuggestedRange.max) : 0;
      const totalBids = typeof data.marketStats?.totalBids === 'number' ? data.marketStats.totalBids : 0;
      const acceptedAvg = typeof data.marketStats?.acceptedAvg === 'number' ? Math.round(data.marketStats.acceptedAvg) : null;
      const winRate = typeof data.providerPerformance?.winRate === 'number' ? data.providerPerformance.winRate : null;
      const won = typeof data.providerPerformance?.won === 'number' ? data.providerPerformance.won : 0;
      const lost = typeof data.providerPerformance?.lost === 'number' ? data.providerPerformance.lost : 0;
      const winInfo = winRate !== null 
        ? `Your win rate: ${winRate}% (${won}/${won + lost})`
        : '';
      container.innerHTML = `
        <div class="calc-ai-suggestion-title">
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>
        ...
        <div class="calc-ai-suggestion-range">$${rangeMin} - $${rangeMax}</div>
        <div class="calc-ai-suggestion-meta">
          Based on ${totalBids} bids in this category${acceptedAvg !== null ? ` (accepted avg: $${acceptedAvg})` : ''}
          ${winInfo ? `<br>${winInfo}` : ''}
        </div>
      `;
    } else {
      container.innerHTML = `
        ...
        <div class="calc-ai-suggestion-meta">Not enough market data for this category yet.</div>
      `;
    }
  } catch (err) {
    console.error('Error loading AI price suggestion:', err);
    container.style.display = 'none';
  }
}

let bidPriceSignalTimer = null;

function setupBidPriceSignal() {
  const priceSelect = document.getElementById('bid-price');
  const customInput = document.getElementById('bid-price-custom');
...
function debouncedPriceSignal(amount) {
  if (bidPriceSignalTimer) clearTimeout(bidPriceSignalTimer);
  bidPriceSignalTimer = setTimeout(() => fetchPriceSignal(amount), 800);
}

async function fetchPriceSignal(amount) {
  const signalEl = document.getElementById('bid-price-signal');
  if (!signalEl) return;
  ...
  const zip = pkg.member_zip || providerProfile?.zip_code || '';
  ...
  try {
    ...
    const colors = {
      below: { bg: 'rgba(46,204,113,0.12)', border: 'rgba(46,204,113,0.3)', text: 'var(--accent-green)' },
      above: { bg: 'rgba(231,76,60,0.12)', border: 'rgba(231,76,60,0.3)', text: 'var(--accent-red)' },
      'in-range': { bg: 'rgba(52,152,219,0.12)', border: 'rgba(52,152,219,0.3)', text: 'var(--accent-blue)' }
    };
    const c = colors[data.direction] || colors['in-range'];

    signalEl.style.display = 'block';
    signalEl.style.background = c.bg;
    signalEl.style.border = `1px solid ${c.border}`;
    signalEl.style.color = c.text;
    signalEl.innerHTML = data.signal;
  } catch (e) {
    signalEl.style.display = 'none';
  }
}

async function draftBidPitch() {
  const btn = document.getElementById('draft-pitch-btn');
  const status = document.getElementById('draft-pitch-status');
  const notesEl = document.getElementById('bid-notes');
  if (!btn || !notesEl) return;

  const pkg = openPackages.find(p => p.id === currentBidPackageId);
  if (!pkg) {
    showToast('Package details not available', 'error');
    return;
  }

  btn.disabled = true;
  btn.textContent = 'Drafting...';
  if (status) { status.style.display = 'inline'; status.textContent = 'AI is writing your pitch...'; }
  ...
  try {
    ...
    const resp = await fetch(`${apiBase}/api/ai/draft-bid`, {
      ...
      body: JSON.stringify({
        ...
        vehicle: pkg.vehicles ? `${pkg.vehicles.year || ''} ${pkg.vehicles.make || ''} ${pkg.vehicles.model || ''}`.trim() : null,
        providerName: providerProfile?.provider_alias || providerProfile?.full_name || null
      })
    });
    ...
    const data = await resp.json();
    if (data.draft) {
      notesEl.value = data.draft;
      notesEl.style.transition = 'box-shadow 0.3s ease';
      notesEl.style.boxShadow = '0 0 0 2px var(--accent-teal)';
      setTimeout(() => { notesEl.style.boxShadow = ''; }, 1500);
      if (status) { status.textContent = 'Draft ready — edit as needed'; }
    }
  } catch (e) {
    showToast('Could not generate draft. Try again.', 'error');
    if (status) status.style.display = 'none';
  } finally {
    btn.disabled = false;
    btn.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;margin-right:4px;"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg> Draft my pitch';
    setTimeout(() => { if (status) status.style.display = 'none'; }, 3000);
  }
}

if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', setupBidPriceSignal);
} else {
  setupBidPriceSignal();
}
